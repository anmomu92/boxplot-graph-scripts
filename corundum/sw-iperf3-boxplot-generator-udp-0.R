...
bp <- boxplot(data_matrix, horizontal=TRUE, main="Conf. 2 - iperf3 UDP throughput (no injection limit)", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
...
svg("boxplots/two-iperf3-boxplot-udp-throughput-0.svg")
boxplot(data_matrix, horizontal=TRUE, main="Conf. 2 - iperf3 UDP throughput (no injection limit)", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
legend("topleft", legend=legend_labels, fill=c("lightblue", "lightgreen"), cex = 0.7)
dev.off()