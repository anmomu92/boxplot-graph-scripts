...
data1 <- read.table("con-switch/iperf3/udp-box-plot-212992-0.txt", header = FALSE)
data2 <- read.table("con-switch/iperf3/udp-box-plot-157286400-0.txt", header = FALSE)
...
bp <- boxplot(data_matrix, horizontal=TRUE, main="Conf. 1 - iperf3 UDP throughput (no injection limit)", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
...
svg("boxplots/switch-iperf3-boxplot-udp-throughput-0.svg")
boxplot(data_matrix, horizontal=TRUE, main="Conf. 1 - iperf3 UDP throughput (no injection limit)", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
legend("topleft", legend=legend_labels, fill=c("lightblue", "lightgreen"), cex = 0.7)
dev.off()