...
bp <- boxplot(data_matrix, horizontal=TRUE, main="Conf. 2 - netperf TCP throughput", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
...
svg("boxplots/two-netperf-boxplot-tcp-throughput.svg")
boxplot(data_matrix, horizontal=TRUE, main="Conf. 2 - netperf TCP throughput", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
legend("topleft", legend=legend_labels, fill=c("lightblue", "lightgreen"), cex = 0.7)
dev.off()