...
data1 <- read.table("tcp-box-plot-212992.txt", header = FALSE)
data2 <- read.table("tcp-box-plot-157286400.txt", header = FALSE)
...
bp <- boxplot(data_matrix, horizontal=TRUE, main="iperf3 TCP throughput", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=FALSE)
...
# Create a legend
legend("topleft", legend=legend_labels, fill=c("lightblue", "lightgreen"), cex = 0.65)
