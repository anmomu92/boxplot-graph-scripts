...
data1 <- read.table("udp-box-plot-212992-0.txt", header = FALSE)
data2 <- read.table("udp-box-plot-157286400-0.txt", header = FALSE)
...
bp <- boxplot(data_matrix, horizontal=TRUE, main="iperf3 UDP throughput (no injection limit)", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
