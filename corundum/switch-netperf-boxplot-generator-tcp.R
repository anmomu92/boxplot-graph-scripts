...
data1 <- read.table("con-switch/netperf/tcp-box-plot-212992.txt", header = FALSE)
data2 <- read.table("con-switch/netperf/tcp-box-plot-157286400.txt", header = FALSE)
...
bp <- boxplot(data_matrix, horizontal=TRUE, main="Conf. 1 - netperf TCP throughput", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
...
svg("boxplots/switch-netperf-boxplot-tcp-throughput.svg")
boxplot(data_matrix, horizontal=TRUE, main="Conf. 1 - netperf TCP throughput", xlab="Throughput (Mbps)", ylab="Buffer size", col=c("lightblue", "lightgreen"), outline=TRUE)
legend("topleft", legend=legend_labels, fill=c("lightblue", "lightgreen"), cex = 0.7)
dev.off()